...
import { addErrorReference, getConceptsForError, removeReference } from "@/lib/actions/references";
...
interface ConceptsForErrorEditorProps {
  errorId: string;
}

// Phase 2 (slice 4): "Related Concepts" sa error detail page — kabaligtaran
// ng direksyon ng WhereIEncounteredIt (dito, hinahanap natin ang
// article/concept na dapat i-link papunta sa error na ito).
export function ConceptsForErrorEditor({ errorId }: ConceptsForErrorEditorProps) {
  ...
  function refresh() {
    getConceptsForError(errorId)
      .then(setConcepts)
      .catch((e) => setError(e instanceof Error ? e.message : "Load failed"));
  }

  useEffect(refresh, [errorId]);
  ...
  function handleAdd(article: ArticleSummary) {
    ...
    startTransition(async () => {
      try {
        await addErrorReference(article.id, errorId);
        setQuery("");
        setResults([]);
        refresh();
      } catch (e) {
    ...
    <div className="space-y-3 rounded-md border border-border p-4">
      ...
      <div className="relative">
        <input
          ...
          placeholder="Maghanap ng concept na may kinalaman sa error na ito…"
